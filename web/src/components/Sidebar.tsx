...
import { Brain, FileText, Settings, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, Edit2, Archive, ArchiveRestore, Trash2, X } from 'lucide-react';
...
interface SidebarProps {
  ...
  onSettings: () => void;
  onPanelChange: (panel: 'memory' | 'settings' | 'notes') => void;
  loading: boolean;
  onClose?: () => void;
}
...
function ConversationItem({
  ...
  return (
    <DropdownMenu open={openMenuId === conv.id} onOpenChange={(open) => onOpenMenu(open ? conv.id : null)}>
      <div
        className={cls(
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-2.5">
        ...
        {showCollapsed && conversations.map(conv => (
          <ConversationItem
            key={conv.id}
            conv={conv}
            selectedId={selectedId}
        ...
        {showCollapsed ? (
          <div className="flex flex-col gap-1 mt-2">
            ...
            <button
              ...
              className={cls(
                'p-2 rounded transition-colors',
                activeView === 'notes' ? 'bg-sidebar-accent text-accent' : 'hover:bg-sidebar-accent'
              )}
              title="Notes"
              aria-label="Notes"
            >

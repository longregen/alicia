      ...
      <div className="relative">
        ...
        {showCategories && (
          <>
            ...
            {/* Dropdown */}
            <div className="absolute right-0 z-20 mt-1 w-full min-w-[140px] rounded border bg-surface shadow-lg p-1">
              {categories.map((category) => (
                <button
                  ...
                  className={cls(
                    'w-full px-3 py-2.5 text-sm text-default text-left rounded cursor-pointer',
                    'transition-all',
                    selectedCategory === category.value
                      ? 'bg-accent-subtle text-accent font-medium'
                      : 'hover:bg-accent-subtle/50 hover:text-accent'
                  )}
                >
                  {category.label}
                </button>

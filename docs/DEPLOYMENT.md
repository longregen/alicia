...
Alicia includes secure CORS (Cross-Origin Resource Sharing) configuration that protects against common security vulnerabilities.

### Security Overview

The CORS middleware has been designed to prevent the critical security vulnerability where `Access-Control-Allow-Origin: *` is combined with `Access-Control-Allow-Credentials: true`, which violates the CORS specification and exposes your application to cross-origin attacks.

### Configuration

CORS origins are configured via the `ALICIA_CORS_ORIGINS` environment variable, which accepts a comma-separated list of allowed origins.

#### Development Mode

By default, Alicia allows `http://localhost:3000` for local development:

```nix
services.alicia = {
  enable = true;
  # CORS will default to http://localhost:3000
};
```
...
echo "https://alicia.example.com,https://app.example.com" | \
  sudo tee /var/secrets/alicia/cors-origins

# Secure the file
sudo chown root:alicia /var/secrets/alicia/cors-origins
sudo chmod 640 /var/secrets/alicia/cors-origins
```
...
```nix
systemd.services.alicia.environment = {
  ALICIA_CORS_ORIGINS = lib.strings.fileContents "/var/secrets/alicia/cors-origins";
};
```
...
```nix
systemd.services.alicia.environment = {
  ALICIA_CORS_ORIGINS = "https://alicia.example.com,https://app.example.com";
};
```

#### Multiple Origins Example

For applications with multiple frontends:

```bash
export ALICIA_CORS_ORIGINS="https://web.example.com,https://mobile.example.com,https://admin.example.com"
```

### How It Works

1. **Origin Validation**: The middleware checks the `Origin` header from incoming requests against the allowed list
2. **Specific Origin Response**: If the origin is allowed, the response includes `Access-Control-Allow-Origin` with that specific origin (never `*`)
3. **Credentials Support**: `Access-Control-Allow-Credentials: true` is only sent when the origin is explicitly allowed
4. **Preflight Handling**: `OPTIONS` requests from allowed origins receive `204 No Content`, while disallowed origins receive `403 Forbidden`

### Security Best Practices

1. **Never use wildcards in production**: Always specify exact origins
2. **Use HTTPS origins**: Prefer `https://` over `http://` for production origins
3. **Keep the list minimal**: Only add origins that genuinely need access
4. **Audit regularly**: Review the allowed origins list periodically
5. **Per-environment configuration**: Use different origins for development, staging, and production
...
curl -H "Origin: https://alicia.example.com" \
...
# Access-Control-Allow-Origin: https://alicia.example.com
...
1. Check that your frontend origin is in `ALICIA_CORS_ORIGINS`
2. Verify the exact origin format (including protocol and port)
3. Check browser console for the actual origin being sent
4. Restart the Alicia service after configuration changes

**Credentials not being sent**:
1. Ensure `credentials: 'include'` is set in fetch/axios requests
2. Verify the origin is in the allowed list
3. Check that both origin and credentials headers are being set

**Preflight requests failing**:
1. Verify `OPTIONS` requests are not being blocked by nginx
2. Check that the request origin is in the allowed list
3. Ensure all required headers are in `Access-Control-Allow-Headers`
...
- [ ] Test CORS configuration with allowed and disallowed origins

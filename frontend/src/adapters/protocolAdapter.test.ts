...
describe('protocolAdapter', () => {
  ...
  describe('handleReasoningStep', () => {
    ...
    it('wraps reasoning content in XML tags with sequence and id attributes', () => {
      ...
      const message = mockStore.messages[createMessageId('msg-001')];
      expect(message.content).toBe(
        '<reasoning data-sequence="0" data-id="reason-001">Let me think about this problem...</reasoning>'
      );
    });
    ...
    it('appends reasoning blocks to existing content', () => {
      ...
      const message = mockStore.messages[messageId];
      expect(message.content).toBe(
        'Some existing content <reasoning data-sequence="1" data-id="reason-001">Additional reasoning...</reasoning>'
      );
    });
    ...
    it('handles multiple reasoning blocks with different sequences', () => {
      ...
      const message = mockStore.messages[messageId];
      expect(message.content).toBe(
        '<reasoning data-sequence="0" data-id="reason-001">First thought</reasoning> <reasoning data-sequence="1" data-id="reason-002">Second thought</reasoning>'
      );
    });
  ...
  // ==========================================================================
  // Feedback Protocol Handlers
  // ==========================================================================

  describe('handleFeedbackConfirmation', () => {
    it('updates feedback store with aggregates', () => {
      const mockSetAggregates = vi.fn();
      vi.mocked(useFeedbackStore.getState).mockReturnValue({
        setAggregates: mockSetAggregates,
      } as any);
      ...
      const confirmation: FeedbackConfirmation = {
        ...
        aggregates: {
          upvotes: 10,
          downvotes: 2,
          specialVotes: { critical: 1 },
        },
        userVote: 'up',
      };

      handleFeedbackConfirmation(confirmation);

      expect(mockSetAggregates).toHaveBeenCalledWith(
        'message',
        'msg-001',
        {
          upvotes: 10,
    ...
    it('handles tool_use target type', () => {
      const mockSetAggregates = vi.fn();
      vi.mocked(useFeedbackStore.getState).mockReturnValue({
        setAggregates: mockSetAggregates,
      } as any);
      ...
      expect(mockSetAggregates).toHaveBeenCalledWith(
        'tool_use',
        'tool-001',
        expect.objectContaining({ upvotes: 5, downvotes: 1 })
      );
    });
  });

  describe('handleNoteConfirmation', () => {
    it('does not throw for successful confirmation', () => {
      const confirmation: NoteConfirmation = {
        noteId: 'note-001',
        messageId: 'msg-001',
        success: true,
      };

      expect(() => handleNoteConfirmation(confirmation)).not.toThrow();
    });

    it('logs warning for failed confirmation', () => {
      const consoleWarnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
      ...
      handleNoteConfirmation(confirmation);

      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'Note confirmation failed for note note-001'
      );
  ...
  describe('handleMemoryConfirmation', () => {
    it('does not throw for successful confirmation', () => {
      ...
      expect(() => handleMemoryConfirmation(confirmation)).not.toThrow();
    });
    ...
    it('logs warning for failed confirmation', () => {
      ...
      handleMemoryConfirmation(confirmation);

      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'Memory action delete failed for memory mem-001'
      );
  ...
  describe('handleServerInfo', () => {
    it('updates server info store with all data', () => {
      const mockSetConnectionStatus = vi.fn();
      const mockSetLatency = vi.fn();
      const mockSetModelInfo = vi.fn();
      const mockSetMCPServers = vi.fn();

      vi.mocked(useServerInfoStore.getState).mockReturnValue({
        setConnectionStatus: mockSetConnectionStatus,
        setLatency: mockSetLatency,
        setModelInfo: mockSetModelInfo,
      ...
      expect(mockSetLatency).toHaveBeenCalledWith(42);
      expect(mockSetModelInfo).toHaveBeenCalledWith({
        name: 'claude-3',
        provider: 'anthropic',
      });
  ...
  describe('handleSessionStats', () => {
    it('updates session stats in server info store', () => {
      const mockSetSessionStats = vi.fn();
      ...
      vi.mocked(useServerInfoStore.getState).mockReturnValue({
        ...
        setModelInfo: vi.fn(),
        setMCPServers: vi.fn(),
        setSessionStats: mockSetSessionStats,
      } as any);
      ...
      expect(mockSetSessionStats).toHaveBeenCalledWith({
        messageCount: 24,
        toolCallCount: 8,
        memoriesUsed: 3,
  ...
  describe('sendFeedback', () => {
    it('sends feedback envelope when sender is available', () => {
      ...
      expect(envelope.type).toBe(MessageType.Feedback);
      expect(envelope.conversationId).toBe('conv-001');
      expect(envelope.body).toBe(feedback);
    });
    ...
    it('warns when no sender is available', () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'Cannot send feedback: no message sender available'
      );
  ...
  describe('sendUserNote', () => {
    it('sends user note envelope when sender is available', () => {
      const mockSender = vi.fn();
      setMessageSender(mockSender);
      ...
      expect(mockSender).toHaveBeenCalledTimes(1);
      const envelope = mockSender.mock.calls[0][0];
      expect(envelope.type).toBe(MessageType.UserNote);
      expect(envelope.body).toBe(note);
    });
    ...
    it('warns when no sender is available', () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'Cannot send note: no message sender available'
      );
  ...
  describe('sendMemoryAction', () => {
    it('sends memory action envelope when sender is available', () => {
      const mockSender = vi.fn();
      setMessageSender(mockSender);
      ...
      sendMemoryAction(action);

      expect(mockSender).toHaveBeenCalledTimes(1);
      const envelope = mockSender.mock.calls[0][0];
      expect(envelope.type).toBe(MessageType.MemoryAction);
      expect(envelope.body).toBe(action);
    });

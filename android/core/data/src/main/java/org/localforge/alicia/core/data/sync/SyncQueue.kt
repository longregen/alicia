...
class SyncQueue @Inject constructor(
    ...
    /**
     * Creates a protocol envelope from a message entity for sync transmission.
     *
     * IMPORTANT: Only user-role messages can be enqueued for sync.
     *
     * This is by design because of the client-server message flow:
     * - User messages: Created locally on the client and synced TO the server
     * - Assistant messages: Generated by the server and pushed TO the client
     * - System messages: Server-side or static configuration, never synced
     *
     * When offline, only user input needs to be persisted and later synced.
     * Assistant responses cannot exist locally before the server processes
     * the user message, so there's nothing to sync upstream.
     *
     * @param message The message entity to convert (must have role = "user")
     * @param stanzaId Protocol stanza ID for the envelope
     * @return Envelope ready for transmission
     * @throws IllegalArgumentException if message role is not "user"
     */

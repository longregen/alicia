...
func TestMessageTypeConstants(t *testing.T) {
	tests := []struct {
		...
		{"TypeErrorMessage", TypeErrorMessage, 1},
		{"TypeUserMessage", TypeUserMessage, 2},
		{"TypeAssistantMessage", TypeAssistantMessage, 3},
		{"TypeAudioChunk", TypeAudioChunk, 4},
		{"TypeReasoningStep", TypeReasoningStep, 5},
		{"TypeToolUseRequest", TypeToolUseRequest, 6},
		{"TypeToolUseResult", TypeToolUseResult, 7},
		{"TypeAcknowledgement", TypeAcknowledgement, 8},
		{"TypeTranscription", TypeTranscription, 9},
		{"TypeControlStop", TypeControlStop, 10},
		{"TypeControlVariation", TypeControlVariation, 11},
		{"TypeConfiguration", TypeConfiguration, 12},
		{"TypeStartAnswer", TypeStartAnswer, 13},
		{"TypeMemoryTrace", TypeMemoryTrace, 14},
		{"TypeCommentary", TypeCommentary, 15},
		{"TypeAssistantSentence", TypeAssistantSentence, 16},
	}
...
func TestStopTypeConstants(t *testing.T) {
	if StopTypeGeneration != "generation" {
		t.Errorf("expected StopTypeGeneration to be 'generation', got %s", StopTypeGeneration)
	}
	if StopTypeSpeech != "speech" {
		t.Errorf("expected StopTypeSpeech to be 'speech', got %s", StopTypeSpeech)
	}
	if StopTypeAll != "all" {
		t.Errorf("expected StopTypeAll to be 'all', got %s", StopTypeAll)
	}
}

func TestToolExecutionConstants(t *testing.T) {
	if ToolExecutionServer != "server" {
		t.Errorf("expected ToolExecutionServer to be 'server', got %s", ToolExecutionServer)
	}
	if ToolExecutionClient != "client" {
		t.Errorf("expected ToolExecutionClient to be 'client', got %s", ToolExecutionClient)
	}
	if ToolExecutionEither != "either" {
		t.Errorf("expected ToolExecutionEither to be 'either', got %s", ToolExecutionEither)
	}
}

func TestAnswerTypeConstants(t *testing.T) {
	if AnswerTypeText != "text" {
		t.Errorf("expected AnswerTypeText to be 'text', got %s", AnswerTypeText)
	}
	if AnswerTypeVoice != "voice" {
		t.Errorf("expected AnswerTypeVoice to be 'voice', got %s", AnswerTypeVoice)
	}
	if AnswerTypeTextVoice != "text+voice" {
		t.Errorf("expected AnswerTypeTextVoice to be 'text+voice', got %s", AnswerTypeTextVoice)
	}
}

func TestVariationTypeConstants(t *testing.T) {
	if VariationTypeRegenerate != "regenerate" {
		t.Errorf("expected VariationTypeRegenerate to be 'regenerate', got %s", VariationTypeRegenerate)
	}
	if VariationTypeEdit != "edit" {
		t.Errorf("expected VariationTypeEdit to be 'edit', got %s", VariationTypeEdit)
	}
	if VariationTypeContinue != "continue" {
		t.Errorf("expected VariationTypeContinue to be 'continue', got %s", VariationTypeContinue)
	}
}
...
func TestErrorCodeConstants(t *testing.T) {
	tests := []struct {
		...
		{"ErrCodeMalformedData", ErrCodeMalformedData, 101},
		{"ErrCodeUnknownType", ErrCodeUnknownType, 102},
		{"ErrCodeConversationNotFound", ErrCodeConversationNotFound, 201},
		{"ErrCodeInvalidState", ErrCodeInvalidState, 202},
		{"ErrCodeToolNotFound", ErrCodeToolNotFound, 301},
		{"ErrCodeToolTimeout", ErrCodeToolTimeout, 304},
		{"ErrCodeInternalError", ErrCodeInternalError, 501},
		{"ErrCodeServiceUnavailable", ErrCodeServiceUnavailable, 503},
	}
...
func TestFeatureConstants(t *testing.T) {
	features := []struct {
		...
		{"FeaturePartialResponses", FeaturePartialResponses},
		{"FeatureAudioOutput", FeatureAudioOutput},
		{"FeatureReasoningSteps", FeatureReasoningSteps},
		{"FeatureToolUse", FeatureToolUse},
	}
...
func TestDefaultToolTimeout(t *testing.T) {
	if DefaultToolTimeout != 30000 {
		t.Errorf("expected DefaultToolTimeout to be 30000, got %d", DefaultToolTimeout)
	}
}

func TestMetaKeyConstants(t *testing.T) {
	if MetaKeyTimestamp != "timestamp" {
		t.Errorf("expected MetaKeyTimestamp to be 'timestamp', got %s", MetaKeyTimestamp)
	}
	if MetaKeyClientVersion != "clientVersion" {
		t.Errorf("expected MetaKeyClientVersion to be 'clientVersion', got %s", MetaKeyClientVersion)
	}
	if MetaKeyTraceID != "messaging.trace_id" {
		t.Errorf("expected MetaKeyTraceID to be 'messaging.trace_id', got %s", MetaKeyTraceID)
	}
	if MetaKeySpanID != "messaging.span_id" {
		t.Errorf("expected MetaKeySpanID to be 'messaging.span_id', got %s", MetaKeySpanID)
	}
}

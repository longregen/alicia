...
func TestSystemPromptVersionRepository_List(t *testing.T) {
	...
	mainVersionsBefore, err := repo.List(ctx, models.PromptTypeMain, 100)
	if err != nil {
		t.Fatalf("List main versions (before) failed: %v", err)
	}
	mainCountBefore := len(mainVersionsBefore)

	toolVersionsBefore, err := repo.List(ctx, models.PromptTypeToolSelection, 100)
	if err != nil {
		t.Fatalf("List tool versions (before) failed: %v", err)
	}
	toolCountBefore := len(toolVersionsBefore)

	// Create 3 main versions with unique hashes
	testTimestamp := time.Now().UnixNano()
	for i := 0; i < 3; i++ {
		version := &models.SystemPromptVersion{
			ID:            idGen.GenerateSystemPromptVersionID(),
			PromptHash:    fmt.Sprintf("main_hash_test_%d_%d", testTimestamp, i),
	...
	// Create 2 tool selection versions
	for i := 0; i < 2; i++ {
		version := &models.SystemPromptVersion{
			ID:            idGen.GenerateSystemPromptVersionID(),
			PromptHash:    fmt.Sprintf("tool_hash_test_%d_%d", testTimestamp, i),
	...
	// List main prompt versions - should have 3 more than before
	versions, err := repo.List(ctx, models.PromptTypeMain, 100)
	if err != nil {
		t.Fatalf("List failed: %v", err)
	}

	expectedMainCount := mainCountBefore + 3
	if len(versions) != expectedMainCount {
		t.Errorf("expected %d main prompt versions (baseline %d + 3), got %d", expectedMainCount, mainCountBefore, len(versions))
	}

	// List tool selection versions - should have 2 more than before
	versions, err = repo.List(ctx, models.PromptTypeToolSelection, 100)
	if err != nil {
		t.Fatalf("List failed: %v", err)
	}

	expectedToolCount := toolCountBefore + 2
	if len(versions) != expectedToolCount {
		t.Errorf("expected %d tool selection versions (baseline %d + 2), got %d", expectedToolCount, toolCountBefore, len(versions))
	}

	// Test limit - should return at most 2
	versions, err = repo.List(ctx, models.PromptTypeMain, 2)
	if err != nil {

...
func TestValidateRequired_Whitespace(t *testing.T) {
	// ValidateRequired only checks for empty string, not whitespace
	// This test documents current behavior
	err := ValidateRequired("   ", "field")
	if err != nil {
		t.Errorf("ValidateRequired() with whitespace should pass (current behavior), got error: %v", err)
	}
}

func TestValidatePositive_Boundary(t *testing.T) {
	// Test exactly at boundary
	err := ValidatePositive(1, "field")
	if err != nil {
		t.Errorf("ValidatePositive(1) should pass, got error: %v", err)
	}

	err = ValidatePositive(0, "field")
	if err == nil {
		t.Error("ValidatePositive(0) should fail, got nil")
	}
}

func TestValidationFunctions_ErrorMessages(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "ValidateID includes entity type",
			validateFunc: func() error {
				return ValidateID("", "conversation")
			},
			expectedInMsg: "conversation",
		},
		{
			name: "ValidateRequired includes field name",
			validateFunc: func() error {
				return ValidateRequired("", "username")
			},
			expectedInMsg: "username",
		},
		{
			name: "ValidateStringLength includes actual length",
			validateFunc: func() error {
				return ValidateStringLength("ab", "field", 5, 10)
			},
			expectedInMsg: "2",
		},
		{
			name: "ValidateRange includes actual value",
			validateFunc: func() error {
				return ValidateRange(15, "score", 0, 10)
			},
			expectedInMsg: "15",
		},
	}
